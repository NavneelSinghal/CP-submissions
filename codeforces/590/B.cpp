...
void solve(int) {
    ...
    cin >> x1 >> y1
        ...
        >> vx >> vy
        >> wx >> wy;
    long double l = 0, r = 1e18L;
    int ITER = 1000;
    auto target = make_pair(x2, y2);
    while (ITER--) {
        long double m = (l + r) / 2;
        long double x3 = (x1 + vx * min(m, t)) + wx * (m - min(m, t));
        long double y3 = (y1 + vy * min(m, t)) + wy * (m - min(m, t));
        auto here = make_pair(x3, y3);
        long double d = dist(here, target);
        if (d < v * m) {
            r = m;
        } else {
            l = m;
