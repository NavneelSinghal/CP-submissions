...
void solve(int) {
    ...
    const int sum_indices = min(n * (n - 1) / 2, k * (k - 1) / 2 + s);
    vector<vector<int>> dp(k + 1, vector<int>(sum_indices + 1, inf));

    // knapsack
    // dp[i][j] = answer when we choose from the first i elements and sum of indices is j
    dp[0][0] = 0;
    for (int i = 0; i < n; ++i) {
        for (int j = k - 1; ~j; --j) {
            for (int sum = sum_indices - i; sum >= 0; --sum) {
                remin(dp[j + 1][sum + i], dp[j][sum] + a[i]);
            }
        }
    }

    int ans = inf;
    for (int i = k * (k - 1) / 2; i <= sum_indices; ++i) {
        remin(ans, dp[k][i]);
    }
