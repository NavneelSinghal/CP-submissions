...
void solve(int) {
    ...
    // d[i][j][s] = distance of i, j from state s
    // 0-1
    
    for (int s = 0; s < 3; ++s) {
    
        vector<vector<int>> d_s(n, vector<int>(m, inf));
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (max({d[0][i][j], d[1][i][j], d[2][i][j]}) < inf) {
                remin(ans, d[0][i][j] + d[1][i][j] + d[2][i][j] - 2 * (a[i][j] == '.'));
            }
        }
    }
    if (ans == inf) cout << -1 << '\n';
