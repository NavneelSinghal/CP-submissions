...
void solve() {
    ...
    int b = (n * m) / 2;
    int w = n * m - b;
    //sum at white - sum at black should be invariant
    //if n * m is odd, then anything would work
    if((n * m) & 1) {
        mint a = r - l + 1;
        cout << power(a, n * m) << endl;
        return;
    }
    //if n * m is even, we need to make sure sum of all elements is even
    int cnteven = r / 2 - (l - 1) / 2;
    int cntodd = (r - l + 1) - cnteven;
    Matrix<mint> M(2, 2);
    M.mat[0][0] = M.mat[1][1] = cnteven;
    M.mat[1][0] = M.mat[0][1] = cntodd;
    M = power(M, n * m, Matrix<mint>::id(2));
    cout << M.mat[0][0] << endl;
}
