...
void solve(){
	...
	if(ans == 0){
		...
		//if(ind.size()&1)
		//for(int i = 1; i < (int)ind.size()-i; i++){
			//freq[ind[i]]++;
			//freq[ind[(int)ind.size()-i]]--;
		//}
		//else{
			//for(int i = 0; i < (int)ind.size()-i-1; i++){
				//freq[ind[i]]++;
				//freq[ind[(int)ind.size()-i-1]]--;
			//}
		//}
		...
		if(freq[ind[ind.size()/2]]&1) odd = ind[ind.size()/2];
		else odd = -1;
		string ret = "";
		REP(i, 26){
			REP(j, freq[i]/2){
