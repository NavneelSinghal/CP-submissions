...
fun PrintWriter.solve() {
    ...
    var maxk2 = -1
    for (i in 0 until n) {
        val (s, t) = readStrings(2)
        val r = readInt()
        val suffix = (s.commonSuffixWith(t)).length.toInt()
        if (r == 1) {
            maxk = min(maxk, suffix)
        } else {
            maxk2 = max(maxk2, suffix)
        }
    }
    if (maxk2 + 1 > maxk) {
        println(0)
        ...
        println(maxk - maxk2)
        println((maxk2 + 1..maxk).joinToString(separator = " "))
    }
}
