...
fun PrintWriter.solve() {
    ...
    var f = 0
    var l = 0
    var a = Array<Pair<Int, Int>>(n) { _ -> Pair<Int, Int>(0, 0) } // first -> guesses that he won, second -> guesses that he lost
    var b = Array<Int>(n * n) { _ -> 0 }
    for (i in 0 until m) {
        var (x, y) = readInts(2)
        x--
        y--
        ...
        a[x] = Pair(a[x].first + 1, a[x].second)
        a[y] = Pair(a[y].first, a[y].second + 1)
        b[x * n + y]++
    }
    var pos = 0
    for (i in 0 until n) {
        for (j in 0 until n) {
            if (j == i) continue
            val c1 = b[i * n + j]
            val c2 = a[i].first + a[j].second - 2 * b[i * n + j]
            if ((f == i) and (l == j)) {
                pos = max(pos, 0)
            } else if ((f == i) or (l == j)) {
                pos = max(pos, c1)
