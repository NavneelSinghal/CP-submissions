...
void solve(int) {
    ...
    for (int i = 0; i < n; ++i) {
        m[sqfree[a[i]]].push_back(a[i]);
    }
    int ans0 = 0; // max size of odd groups and even groups initially
    int ans1 = 0; // max size of odd groups and final even group
    int cursum = 0;
    for (auto &[p, v] : m) {
        if (p == 1) {
            cursum += v.size();
