...
mint multiplier(int a, int b, int c, int d) {
    int eq1 = int(a == b) + int(c == d);
    int eq = int(b == c) + eq1;
    if (eq == 0) {
        return 24;
    } else if (eq == 1) {
...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        vector<mint> v(k + 1);
        vector<mint> v2(n + 1);
        for (int i = 0; i <= n; ++i) v2[i] = ifact[i] * ifact[n - i];
        for (int s = 0; s <= k; ++s) {
            mint ans = 0;
            mint factor_0 = fact[n].pow(4) * ifact[4 * n] * fact[s] *
                            fact[4 * n - s] / (4 * n - s);
            int m1 = min(n, s);
            for (int a = max(0, s - n - n - n); a <= m1; ++a) {
                mint factor_a = (n - a) * factor_0 * v2[a];
                int m2 = min(n, s - a);
                for (int b = max(a, s - a - n - n); b <= m2; ++b) {
                    mint factor_b = factor_a * v2[b];
                    int m3 = min(n, s - a - b);
                    for (int c = max(b, s - a - b - n); c <= m3; ++c) {
                        int d = s - a - b - c;
                        if (d < c) break;
                        mint factor_c = factor_b * v2[c] * v2[d];
                        ans += multiplier(a, b, c, d) * factor_c;
                    }
                }
            }
            v[s] = ans;
        ...
        mint ans = 0;
        for (int i = 1; i <= 4 * n; ++i) ans += v[min(i - 1, k)];
        cout << ans << '\n';
    }
}
