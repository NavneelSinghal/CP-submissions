...
void solve() {
    ...
    rep(i, n) {
        rep(j, 201) {
            numcost[i+1][j] = numcost[i][j] + (a[i].S == j);
        }
    }
    int cur = 1;
    vi prev(n);
    ...
    repd(i, n) {
        //set maximum = a[i].F
        //for that remove all the legs on the right - cost += a[i+1].S (if i != n - 1)
        //now count how many elements to the left are equal to this
        if(i != n - 1) cost += a[i+1].S;
        int remaining = prev[i];
        int tot = i + 1;
        int target = 2 * remaining - 1;
        debug(i);
        if (tot > target) {
            int curcost = cost;
            int end = i - remaining + 1;
            debug(end);
            rep(j, 201) {
                int something = min(tot - target, numcost[end][j]);
                if(j < 5) debug(j, something);
                curcost += something * j;
                tot -= something;
            }
            debug(curcost);
