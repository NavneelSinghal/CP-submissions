...
void solve(int case_no) {
    ...
    vector<string> a(n);
    rep(i, n) cin >> a[i];
    vvi up(n, vi(m)), dn(n, vi(m));
    rep(j, m) {
        up[0][j] = 0;
        rep(i, 1, n) {
            if(a[i-1][j] == a[i][j]) up[i][j] = 1 + up[i-1][j];
        }
    }
    rep(j, m) {
        dn[n-1][j] = 0;
        repd(i, n-1) {
            if(a[i][j] == a[i+1][j]) dn[i][j] = 1 + dn[i+1][j];
        }
    }
    rep(i, n) rep(j, m) {
        up[i][j] = min(up[i][j], dn[i][j]);
    }
    
    vvi(1).swap(dn);
    
    vvi lf(n, vi(m)), rt(n, vi(m));
    
    rep(i, n) {
        lf[i][0] = 0;
        rep(j, 1, m) {
            if (a[i][j - 1] == a[i][j]) {
                lf[i][j] = min(up[i][j], 1 + lf[i][j - 1]);
            }
        }
    }
