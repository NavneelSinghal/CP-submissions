...
void solve(int case_no) {
    ...
    int q;
    cin >> q;
    set<int> freq[10]; //freq[i] = number of elements with frequency i, and freq[8] is the number of elements with frequency >= 8
    for (auto f : m) {
        freq[min(8ll, f.S)].insert(f.F);
    }
    rep(i, q) {
        char c;
        cin >> c;
