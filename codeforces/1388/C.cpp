...
void solve(int case_no) {
    ...
    // check subtree size parity and h parity
    // if match for all, do another dfs to check whether bad <= total bad in the subtrees
    vi siz(n);
    vi good(n);
    ...
    function<void(int, int)> dfs = [&] (int u, int p) {
        ...
        if ((siz[u] + h[u]) & 1) works = false;
        good[u] = (siz[u] + h[u]) >> 1;
        if (good[u] < 0 || good[u] > siz[u]) works = false;
        if (sum > good[u]) works = false;
    };
