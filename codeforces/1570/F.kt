...
fun PrintWriter.solve() {
    val n = readInt()
    val m = readInt()
    val a = Array<Array<Int>>(n, { Array<Int>(m, { _ -> readInt() }) })
    val b = Array<Array<Int>>(n, { Array<Int>(m, { _ -> 0 }) })
    var l = MutableList<Pair<Int, Int>> (0, { _ -> Pair(-1, -1) });
    for (i in 1..n - 1) {
        for (j in 1..m - 1) {
            if ((a[i - 1][j - 1] == 1) and (a[i][j - 1] == 1) and (a[i - 1][j] == 1) and (a[i][j] == 1)) {
                l.add(Pair(i, j))
                b[i][j] = 1
                b[i - 1][j] = 1
                b[i][j - 1] = 1
