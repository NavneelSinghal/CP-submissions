...
fun PrintWriter.solve() {
    println("? " + (1..100).joinToString(separator = " "))
    flush()
    val x = readInt()
    val tophalf = (x shr 7) shl 7
    println("? " + (128..12800 step 128).joinToString(separator = " "))
    flush()
    val y = readInt()
    val bottomhalf = y and ((1 shl 7) - 1)
    println("! " + (tophalf + bottomhalf).toString())
}
