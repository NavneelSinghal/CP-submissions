...
void solve(int) {
    ...
    int ans = 0;
    int last_b_index = 0, last_c_index = 0;

    for (int i = 1; i <= n; ++i) {
        if (a[i] == a[last_b_index]) {
            ans += a[last_c_index] != a[i];
            last_c_index = i;
        } else if (a[i] == a[last_c_index]) {
            ans += a[last_b_index] != a[i];
            last_b_index = i;
        } else {
            if (pos[last_b_index] < pos[last_c_index]) {
                ans += a[last_b_index] != a[i];
                last_b_index = i;
            } else {
                ans += a[last_c_index] != a[i];
