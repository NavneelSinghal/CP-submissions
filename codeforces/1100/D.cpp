...
void solve(int) {
    ...
    int _ = 2;
    while (_--) {
        while (king_x != mid_x || king_y != mid_y) {
            int move_x = king_x + cmp(mid_x, king_x);
            int move_y = king_y + cmp(mid_y, king_y);
            if (a[move_y][move_x]) move_x = king_x;
            cout << move_x << ' ' << move_y << '\n';
            cout.flush();
            int k, rook_x, rook_y;
            cin >> k >> rook_x >> rook_y;
