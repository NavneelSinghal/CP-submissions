...
auto main() -> signed {
    ...
    auto solve = [&](int t) -> void {
        ...
        stable_sort(begin(id), end(id), [](int a, int b) {
            cout << 1 << ' ' << a << ' ' << b << endl;
            int x;
            cin >> x;
            return x;
        });
        auto query2 = [&](int x, int ptr) {
            cout << 2 << ' ' << x << ' ' << ptr + 1 << '\n';
            for (int i = 0; i <= ptr; ++i) cout << id[i] << ' ';
            cout << endl;
            cin >> x;
            return x;
        };
        ...
        int left_ptr = n - 2;
        for (int right_ptr = n - 1; right_ptr >= 0; --right_ptr) {
            if (left_ptr == right_ptr) {
                for (int i = 0; i <= left_ptr; ++i)
                    for (int j = right_ptr + 1; j < n; ++j)
                        ans[id[j]][id[i]] = 0;
                --left_ptr;
            }
            while (query2(id[right_ptr], left_ptr)) --left_ptr;
        }
        cout << 3 << '\n';
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
