...
signed main(){
	...
	while(t--){
		...
		REP(i, n-1){
			//do something with this and the first letter of the last string
			int left = 0;
			int right = a[i].size()-1;
			while(left < right){
				if(a[i][left] != a[i][right]){
					if(a[i][left] == a[n-1][0]){
						swap(a[i][right], a[n-1][0]);
					}
					else{
						swap(a[i][left], a[n-1][0]);
					}
				}
				left++;
				right--;
		...
		//now see what can be done with the last element
		int ww = 0;
		for(auto w : a[n-1]){
			ww += w-'0';
		}
