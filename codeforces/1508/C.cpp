...
auto main() -> signed {
    ...
    auto solve = [&](int t) -> void {
        ...
        ll remaining = 1LL * n * (n - 1) / 2 - m;
        set<int> left_vertices;
        dsu complete(n), existing(n);
        vector<vector<int>> g(n);
        vector<tuple<int, int, int>> edges;
        edges.reserve(m);
        int xor_all = 0;

        for (int i = 0; i < n; ++i) { left_vertices.emplace(i); }
        ...
        for (int i = 0; i < n; ++i) {
            if (left_vertices.find(i) != left_vertices.end()) {
                y_combinator([&](auto dfs, int u) -> void {
                    left_vertices.erase(u);
                    sort(begin(g[u]), end(g[u]));
                    int v = -1;
                    while (true) {
                        auto it = left_vertices.lower_bound(v);
                        if (it == left_vertices.end()) return;
                        v = *it;
                        if (!binary_search(begin(g[u]), end(g[u]), v)) {
                            complete.union_sets(u, v);
                            --remaining;
                            dfs(v);
                        }
                        ++v;
        ...
        for (auto &[w, u, v] : edges) {
            if (complete.union_sets(u, v)) {
                ans += w;
                existing.union_sets(u, v);
            } else if (existing.union_sets(u, v)) {
                remin(xor_all, w);
            }
        }
