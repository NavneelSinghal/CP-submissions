...
auto main() -> signed {
    ...
    auto solve = [&](int t) -> void {
        ...
        for (int i = 1; i + lg < n; ++i) { cout << i << ' '; }
        int offset = n - lg;
        --k;
        ll sum = 0;
        int completed = offset - 1;
        while (offset < n) {
            int cur = offset;
            while (sum + shift(n - cur - 1) <= k && cur <= n) {
                sum += shift(n - cur - 1);
                cur++;
            }
            for (int i = cur; i >= offset; --i) cout << i << ' ';
            completed += cur - offset + 1;
            offset = cur + 1;
        }
        if (completed < n) cout << n;
        cout << '\n';
    };
