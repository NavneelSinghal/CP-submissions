...
auto main() -> signed {
    ...
    auto solve = [&](int t) -> void {
        ...
        vector<int> c0(3), c1(3);
        for (int i = 0; i < 3; ++i) {
            c0[i] = count(begin(s[i]), end(s[i]), '0');
            c1[i] = count(begin(s[i]), end(s[i]), '1');
        }
        for (int i = 0; i < 3; ++i) {
            int j = (i + 1) % 3;
            char c;
            ...
            for (int k = 0; k < 2 * n; ++k) {
                if (s[i][k] == c) pos1.push_back(k);
                if (s[j][k] == c) pos2.push_back(k);
            }
            assert(pos1.size() >= n);
            assert(pos2.size() >= n);
            string ans = "";
            ...
            int pr2 = 0;
            for (int k = 0; k < n; ++k) {
                ans += s[i].substr(pr1, pos1[k] - pr1);
                ans += s[j].substr(pr2, pos2[k] - pr2);
                ans += c;
                pr1 = pos1[k] + 1;
                pr2 = pos2[k] + 1;
            }
            ans += s[i].substr(pos1[n - 1] + 1);
            ans += s[j].substr(pos2[n - 1] + 1);
            cout << ans << '\n';
            return;
        }
        assert(false);
