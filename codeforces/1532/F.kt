...
fun PrintWriter.solve() {
    val n = readInt()
    val l = readStrings(2 * n - 2)
    val possible = MutableList(0) { _ -> 0 }
    ...
    var first = false
    var second = false
    if (l[possible[1]].startsWith(l[possible[0]].subSequence(1, n - 1))) {
        first = true
    }
    if (l[possible[0]].startsWith(l[possible[1]].subSequence(1, n - 1))) {
        second = true
    }
    fun f() : CharArray {
        originalString = l[possible[0]] + l[possible[1]][n - 2]
        val prefixes = MutableList(n) { _ -> -1 }
        val suffixes = MutableList(n) { _ -> -1 }
        val ans = CharArray(2 * n - 2) { _ -> 'U' }
        for (i in 0 until 2 * n - 2) {
            val len = l[i].length
            val isPrefix = l[i] == originalString.substring(0, len)
            val isSuffix = l[i] == originalString.substring(n - len, n)
            if (isPrefix && prefixes[len] == -1) {
                prefixes[len] = i
                ans[i] = 'P'
            } else if (isSuffix && suffixes[len] == -1) {
                suffixes[len] = i
                ans[i] = 'S'
            } else {
                assert(false)
