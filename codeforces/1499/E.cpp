...
void solve(int) {
    ...
    int n = x.size();
    int m = y.size();
    // dp[i][j][where] = to answer stuff with r1 = i, r2 = j
    // and comes from 'where'
    mint dp[n + 1][m + 1][2];
    for (int i = 0; i <= n; ++i)
        for (int j = 0; j <= m; ++j)
            for (int l = 0; l < 2; ++l) dp[i][j][l] = 0;

    vector<int> len_x(n + 1, 1), len_y(m + 1, 1);
    len_x[1] = 1;
    len_y[1] = 1;
    for (int i = 2; i <= n; ++i) {
        if (x[i - 1] != x[i - 2]) len_x[i] = 1 + len_x[i - 1];
    }
    for (int j = 2; j <= m; ++j) {
        if (y[j - 1] != y[j - 2]) len_y[j] = 1 + len_y[j - 1];
    }
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            // suffix of size 1
            // chunk of size 1
            if (x[i - 1] != y[j - 1]) dp[i][j][1] += len_x[i];
            // suffix of size > 1
            // chunk of size 1
            if (x[i - 1] != y[j - 1]) dp[i][j][1] += dp[i][j - 1][0];
            // chunk of size > 1
            if (j >= 2 && y[j - 2] != y[j - 1]) dp[i][j][1] += dp[i][j - 1][1];

            if (y[j - 1] != x[i - 1]) dp[i][j][0] += len_y[j];
            if (y[j - 1] != x[i - 1]) dp[i][j][0] += dp[i - 1][j][1];
            if (i >= 2 && x[i - 2] != x[i - 1]) dp[i][j][0] += dp[i - 1][j][0];
        }
    }
