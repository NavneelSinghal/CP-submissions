...
void solve(int case_no) {
    ...
    const int mxx = dp[0][0][0];
    dp[0][0][0] = 0;
    int k = tr.size(), l = p.back();
    rep(i, k) rep(j, l + 1) rep(s, (1 + (n * (n - 1) >> 1))) {
        if (dp[i][j][s] == mxx) continue;
        rep(q, j, l + 1) remin(dp[i + 1][q][s + abs(q - p[i])], dp[i][j][s] + (q - j) * (q - j));
    }
    int mn = mxx;
    rep(s, (1 + (n * (n - 1) >> 1))) {
        remin(mn, dp[k][l][s]);
