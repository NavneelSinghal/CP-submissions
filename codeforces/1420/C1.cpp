...
void solve(int case_no) {
    ...
    vi a(n);
    rep(i, n) cin >> a[i];
    // oddmin oddmax evenmin evenmax
    function<pair<pii, pii>(vi&)> f = [&] (vi& a) {
        if (a.size() == 1) return mp(mp(a[0], a[0]), mp(LINF, -LINF));
        int n = a.size();
        vi w[2];
        rep(i, (n >> 1)) w[0].pb(a[i]);
        rep(i, (n - (n >> 1))) w[1].pb(a[i + (n >> 1)]);
        assert(w[0].size() + w[1].size() == n);
        auto pl = f(w[0]);
        auto pr = f(w[1]);
        // if even: odd - odd or even + even
        // even
