...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        using Base = int;
        const Node id_node = {0, 0, 0, 0, 0, 0};
        auto make_node = [](Base b, int) {
            return Node{b, b, 1, 1, 1, 1};
        };
        ...
        auto apply_update = [&make_node](const Update& u,
                                         const Node& n) -> Node {
            if (u == 0) return n;
            return make_node(u, 0);
        };
