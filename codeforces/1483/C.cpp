...
template <typename node = long long, typename T = long long, node N = 0>
struct SegTree {
    // change this
    node combine(node n1, node n2) { return max(n1, n2); }
...
void solve(int) {
    ...
    for (int i = 1; i <= n; ++i) {
        dp[i] = st.query(prv[i], i - 1) + b[i];
        if (prv[i]) remax(dp[i], dp[prv[i]]);
        st.update(i, dp[i]);
    }
    cout << dp[n] << '\n';
}
