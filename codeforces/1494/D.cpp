...
void solve(int) {
    ...
    vector<int> par((n << 1) - 1), sal((n << 1) - 1), id_(n);
    iota(begin(id_), end(id_), 0);
    int nxt = n;

    function<int(vector<int> &)> get_root = [&](vector<int> &id) {
        if (id.size() == 1) {
            sal[id.back()] = a[id.back()][id.back()];
            return id.back();
        }
        int n = id.size();
        int mx = 0;
        for (int i = 0; i < n; ++i) remax(mx, a[id[0]][id[i]]);
        vector<int> level_vertices;
        ...
        for (int i = 0; i < n; ++i) {
            if (!vis[i]) {
                ...
                for (int j = i; j < n; ++j) {
                    if (a[id[i]][id[j]] != mx) {
                        to_work.emplace_back(id[j]);
                        vis[j] = true;
                    }
                }
                level_vertices.push_back(get_root(to_work));
            }
        }
        for (int v : level_vertices) par[v] = nxt;
        sal[nxt++] = mx;
        return nxt - 1;
    };
