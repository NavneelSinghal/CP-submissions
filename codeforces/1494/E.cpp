...
void solve(int) {
    int n, m;
    cin >> n >> m;
    unordered_map<pair<int, int>, char, pair_hash> edges;
    int odd = 0, even = 0;
    for (int i = 0; i < m; ++i) {
        char c;
        cin >> c;
        if (c == '+') {
            ...
            char d;
            cin >> u >> v >> d;
            edges[make_pair(u, v)] = d;
            auto it_rev = edges.find(make_pair(v, u));
            if (it_rev != edges.end()) {
                ++odd;
                if (it_rev->second == d) ++even;
            }
        } else if (c == '-') {
            int u, v;
            cin >> u >> v;
            ...
            if (it_rev != edges.end()) {
                --odd;
                if (it_rev->second == it->second) --even;
            }
