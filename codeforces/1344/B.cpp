...
struct Modular {
    ...
    friend Modular mexp(Modular a, long long e) {
        //if(e >= MOD) e %= (MOD - 1); //works only for prime mods
...
int check (string a) {
    ...
    if (checkw) return 1; //all white column
    bool started = false;
    bool done = false;
    rep (i, n) {
        if (a[i] == '#' && !started) started = true;
        else if (a[i] == '#' && done) return 0; //doesnt work
        else if (a[i] == '.' && started) done = true;
    }
    return 2; //all black segment exists
}
...
void dfs (int i, int j, vector<string> &s, vvi &vis) {
    vis[i][j] = 1;
    for (auto x : dir) {
        int ii = i + x.F;
        int jj = j + x.S;
        if(check(ii, jj) && s[ii][jj] == '#' && vis[ii][jj] == 0) dfs(ii, jj, s, vis);
    }
}
...
void solve() {
    ...
    //now count number of components
    int ans = 0;
    rep (i, n) rep (j, m) {
        if (!vis[i][j] && v[i][j] == '#') ans++, dfs(i, j, v, vis);
    }
    cout << ans << "\n";
