...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        auto apply_update = [](Update u, Node n) -> Node {
            if (n == id_node) return n;
            return u + n;
        };
        ...
        for (int i = n - 1; i >= 0; --i) {
            st.update(i, n, a[i]);
            int pos = st.max_right(i, [&](Node n) { return n >= -s; });
#ifdef DEBUG
            vector<ll> cur(n);
            for (int j = 0; j < n; ++j) cur[j] = st.query(j, j + 1);
            debug(cur);
#endif
            debug(i, pos);
            int len = pos - i;
