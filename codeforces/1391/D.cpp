...
void solve(int case_no) {
    ...
    rep(i, 8) rep(j, 8) {
        b3[i][j] = 1;
        rep(s, 2) {
            int w = ((i >> s) & 1) + ((i >> (s + 1)) & 1) + ((j >> s) & 1) + ((j >> (s + 1)) & 1);
            b3[i][j] &= (w & 1);
        }
    }
    rep(i, 4) rep(j, 4) {
    ...
    char c;
    rep(i, n) rep(j, m) cin >> c, a[i+1][j+1] = c - '0';
    ...
    if (n == 3) {
        rep(i, 1, m + 1) {
            int mask = a[1][i] + (a[2][i] << 1) + (a[3][i] << 2);
            rep(cur, 8) {
                dp[i][cur] = INF;
                rep(prev, 8) {
                    if (!b3[prev][cur]) continue;
                    dp[i][cur] = min(dp[i][cur], dp[i - 1][prev] + __builtin_popcount(cur ^ mask));
                }
            }
        }
        rep(i, 8) ans = min(ans, dp[m][i]);
    }
    else {
        rep(i, 1, m + 1) {
            int mask = a[1][i] + (a[2][i] << 1);
            rep(cur, 4) {
                dp[i][cur] = INF;
                rep(prev, 4) {
                    if (!b2[prev][cur]) continue;
                    dp[i][cur] = min(dp[i][cur], dp[i - 1][prev] + __builtin_popcount(cur ^ mask));
                }
            }
        }
        rep(i, 4) ans = min(ans, dp[m][i]);
    }
    cout << ans << endl;
}
