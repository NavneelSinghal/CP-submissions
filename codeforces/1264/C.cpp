...
void solve(int) {
    ...
    vector<mint> p(n);
    for (auto &x : p) cin >> x.value;
    
    vector<mint> pref_prod(n + 1);
    vector<mint> pref_prod_sum(n + 1);
    pref_prod[0] = 1;
    pref_prod_sum[0] = 0;
    for (int i = 0; i < n; ++i) {
        pref_prod[i + 1] = pref_prod[i] * p[i] / 100;
        pref_prod_sum[i + 1] = pref_prod_sum[i] + pref_prod[i];
    }

    // [l, r)
    // check bounds
    auto query = [&](int l, int r) {
        return (pref_prod_sum[r] - pref_prod_sum[l]) / pref_prod[r];
    };

    set<int> checkpoints;
    set<int> neg_checkpoints;
    vector<int> is_checkpoint(n + 1);
    
    checkpoints.insert(0);
    checkpoints.insert(n);
    neg_checkpoints.insert(0);
    neg_checkpoints.insert(-n);
    is_checkpoint[0] = 1;
    is_checkpoint[n] = 1;

    mint ans = query(0, n);
        // pref_prod_sum[n] / pref_prod[n];
    while (q--) {
        int u;
        cin >> u;
        --u;
        auto it = checkpoints.upper_bound(u);
        auto neg_it = neg_checkpoints.upper_bound(-u);
        int rgt = *it;
        int lft = -(*neg_it);
        if (!is_checkpoint[u]) {
            ans -= query(lft, rgt);
            ans += query(lft, u) + query(u, rgt);
            checkpoints.insert(u);
            neg_checkpoints.insert(-u);
        } else {
            ans += query(lft, rgt);
            ans -= query(lft, u) + query(u, rgt);
            checkpoints.erase(u);
            neg_checkpoints.erase(-u);
        }
        is_checkpoint[u] ^= 1;
        cout << ans << '\n';
    }
