...
void solve(int) {
    ...
    cin >> a;
    int n = (int)a.size();
    vector<int> pre_q(n + 1);
    for (int i = 0; i < n; ++i) pre_q[i + 1] = pre_q[i] + (a[i] == '?');
    vector<vector<mint>> dp(n, vector<mint>(n));
    for (int d = 1; d < n; ++d) {
        for (int i = 0; i + d < n; ++i) {
            int j = i + d;
            if (a[i] == ')' || a[i] == '?') dp[i][j] += dp[i + 1][j];
            if (a[j] == '(' || a[j] == '?') dp[i][j] += dp[i][j - 1];
            if (!(a[i] == '(' || a[j] == ')')) dp[i][j] -= dp[i + 1][j - 1];
            if (!(a[i] == ')' || a[j] == '('))
                dp[i][j] +=
                    dp[i + 1][j - 1] + pwr(mint(2), pre_q[j] - pre_q[i + 1]);
        }
    }
    cout << dp[0][n - 1] << '\n';
}
