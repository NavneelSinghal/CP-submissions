...
void solve(int) {
    ...
    vector<ll> a(n);
    for (auto &x : a) cin >> x;
    vector<ll> pre_add(n), pre(n);
    partial_sum(begin(a), end(a), begin(pre_add), [](ll a, ll b){return a + b;});
    partial_sum(begin(pre_add), end(pre_add), begin(pre), [](ll a, ll b){return max(a, b);});
    while (m--) {
        ll x;
        cin >> x;
        auto it = lower_bound(begin(pre), end(pre), x);
        if (it == end(pre)) {
            if (pre_add.back() <= 0) {
                cout << -1 << ' ';
            } else {
                assert(x > pre.back());
                ll cycles = (x - pre.back()) / pre_add.back();
                while (true) {
                    ll w = x - 1LL * cycles * pre_add.back();
                    auto it = lower_bound(begin(pre), end(pre), w);
                    if (it != end(pre)) {
                        cout << 1LL * cycles * n + (it - begin(pre)) << ' ';
                        break;
                    }
                    cycles++;
                }
