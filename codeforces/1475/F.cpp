...
void solve(int) {
    ...
    for (auto &g : x) g.resize(n);
    
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            x[i][j] = (a[i][j] - '0') ^ (b[i][j] - '0');
        }
    }
    ...
    vector<int> coloured_cols, uncoloured_cols;

    for (int i = 0; i < n; ++i) {
        if (x[0][i] == 1) {
            coloured_cols.push_back(i);
        } else {
            uncoloured_cols.push_back(i);
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        vector<int> col(2), uncol(2);
        for (int j : coloured_cols) {
            col[x[i][j]] = 1;
        }
        for (int j : uncoloured_cols) {
            uncol[x[i][j]] = 1;
        }
        if (col[0] == 0 && uncol[1] == 0 || col[1] == 0 && uncol[0] == 0) {
            continue;
        } else {
            works = false;
            break;
