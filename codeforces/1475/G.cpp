...
void solve(int) {
    ...
    vector<int> chain_length(maxa + 1, 0);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        chain_length[a[i]] = max(chain_length[a[i]], 1);
        for (auto d : divisors[a[i]]) {
            chain_length[a[i]] = max(chain_length[a[i]], 1 + chain_length[d]);
        }
    }

    cout << n - *max_element(begin(chain_length), end(chain_length)) << '\n';
