...
fun PrintWriter.solve() {
    ...
    val m = readInt()
    val s = readStrings(n)
    val ans = LongArray(6) { _ -> 0L}
    val stack = IntArray(m + 1) { _ -> 0 }
    val maskans = LongArray(32) { _ -> 0L}
    for (mask in 1..31) {
        val height = IntArray(m) {_ -> 0}
        for (i in 0 until n) {
            stack[0] = -1
            var stackSz = 1
            var suitable = 0L
            for (j in 0 until m) {
                if (((1 shl (s[i][j] - 'A')) and mask) != 0) {
                    height[j]++
                } else {
                    height[j] = 0
                }
                while (stackSz > 1 && height[j] <= height[stack[stackSz - 1]]) {
                    suitable -= height[stack[stackSz - 1]] * (stack[stackSz - 1] - stack[stackSz - 2])
                    stackSz -= 1
                }
                suitable += height[j] * (j - stack[stackSz - 1])
                stack[stackSz] = j
                ++stackSz
                maskans[mask] += suitable
            }
        }
        for (submask in 1 until mask) {
            if ((mask and submask) == submask) maskans[mask] -= maskans[submask]
        }
        var total = 0
        for (i in 0..4) {
