...
void solve(int) {
    ...
    vector<int> end(n);
    for (int i = m; i <= m + n; ++i)
        if (pi[i] == m) end[i - m - 1] = 1;
    vector<mint> a(n + 1), ways_till(n + 1), sum_ways_till(n + 1);
    for (int i = 0; i < n; ++i) {
        if (!end[i]) {
            a[i + 1] = a[i];
        } else {
            a[i + 1] = sum_ways_till[i - m + 1] + i - m + 2;
        }
        ways_till[i + 1] = ways_till[i] + a[i + 1];
        sum_ways_till[i + 1] = sum_ways_till[i] + ways_till[i + 1];
    }
    cout << ways_till[n] << '\n';
}
