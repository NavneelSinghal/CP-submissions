...
void solve(int) {
    ...
    if (loc[0].first == loc[1].first) {
        int nextx = (loc[0].first + 1) % n;
        a[nextx][loc[1].second] = '*';
        a[nextx][loc[0].second] = '*';
    } else if (loc[0].second == loc[1].second) {
        int nexty = (loc[0].second + 1) % n;
        a[loc[0].first][nexty] = '*';
        a[loc[1].first][nexty] = '*';
    } else {
        a[loc[0].first][loc[1].second] = '*';
        a[loc[1].first][loc[0].second] = '*';
    }
    for (auto &x : a) cout << x << '\n';
}
