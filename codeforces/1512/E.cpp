...
void solve(int) {
    ...
    cin >> n >> l >> r >> s;
    int len = r - l + 1;
    auto mx = [](int n, int r) { return r * (2 * n + 1 - r) / 2; };
    auto mn = [](int r) { return r * (r + 1) / 2; };
    vector<int> range(len);
    for (int i = n; i; --i) {
        if (len && mx(i, len) >= s && mn(len - 1) + i <= s) {
            range[--len] = i;
            s -= i;
        }
    }
    if (s) {
        ...
        for (auto x : range) ans.push_back(x);
        for (int i = r + 1; i <= n; ++i) {
            while (cur <= n && vis[cur]) ++cur;
            ans.push_back(cur);
            vis[cur] = 1;
