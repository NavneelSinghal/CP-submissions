...
void solve(int case_no) {
    ...
    rep(i, 2) rep(j, n) cin >> a[i][j], --a[i][j];
    
    vvi pos(n);
    rep(i, 2) rep(j, n) pos[a[i][j]].pb(j);
    ...
    rep(i, n) {
        ...
        int col1 = pos[i][0], col2 = pos[i][1];
        if (col1 != col2) {
            int work = (a[0][col1] != i) == (a[0][col2] != i);
            g[col1].eb(col2, work);
            g[col2].eb(col1, work);
        }
    }
    ...
    function<void(int, int, int)> dfs = [&](int v, int c, int co) {
        col[v] = c;
        if (col[v] == 0) cnt0++;
        else cnt1++;
        comp[v] = co;
    ...
    cout << ans << "\n";
    rep(i, n) {
        if (col[i] != (colcnt[comp[i]].F < colcnt[comp[i]].S)) {
            cout << i + 1 << " ";
        }
    }
    cout << "\n";
