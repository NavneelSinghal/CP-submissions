...
void solve(int) {
    ...
    for (auto &x : d) sort(begin(x), end(x));
    vector<ll> dp(2), pr(2);
    // 0 - ends at last location
    // 1 - ends at starting location
    for (int i = 0; i < n; ++i) {
        if (d[i].empty()) continue;
        vector<ll> dp_here(2, linf);
        // dir is previous direction
        for (int dir = 0; dir < 2; ++dir) {
            ll x = dp[dir];
            ll prev_pos = pr[dir];
            int nxt_pos = lower_bound(begin(d[i]), end(d[i]), prev_pos) - begin(d[i]);
            ll cost0 = x;
            if (nxt_pos == d[i].size()) {
                cost0 += prev_pos - d[i].front();
            } else {
                cost0 += abs(d[i].back() - prev_pos) + d[i].back() - d[i].front();
            }
            remin(dp_here[1], cost0);
            ll cost1 = x;
            if (nxt_pos == 0) {
                cost1 += d[i].back() - prev_pos;
            } else {
                cost1 += d[i].back() - d[i].front() + abs(d[i].front() - prev_pos);
            }
            remin(dp_here[0], cost1);
        }
        dp = dp_here;
        pr[0] = d[i].back();
        pr[1] = d[i].front();
    }
    cout << min(abs(pr[0]) + dp[0], abs(pr[1]) + dp[1]) << '\n';
}
