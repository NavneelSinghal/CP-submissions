...
multiset<int> ratings[200005], high;

void insert(int i) {
    if (!ratings[b[i]].empty()) {
        high.erase(high.find(*ratings[b[i]].rbegin()));
    }
    ratings[b[i]].insert(a[i]);
    high.insert(*ratings[b[i]].rbegin());
}

void remove(int i) {
    high.erase(high.find(*ratings[b[i]].rbegin()));
    ratings[b[i]].erase(ratings[b[i]].find(a[i]));
    if (!ratings[b[i]].empty()) {
        high.insert(*ratings[b[i]].rbegin());
    }
}
