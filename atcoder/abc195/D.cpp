...
void solve(int) {
    ...
    for (auto &i : x) cin >> i;
    vector<int> bag_order(n);
    iota(begin(bag_order), end(bag_order), 0);
    sort(begin(bag_order), end(bag_order),
         [&](int i, int j) { return bags[i].second > bags[j].second; });
    while (q--) {
        int l, r;
        cin >> l >> r;
        --l;
        vector<int> boxes(m);
        iota(begin(boxes), end(boxes), 0);
        boxes.erase(begin(boxes) + l, begin(boxes) + r);
        sort(begin(boxes), end(boxes),
             [&](int i, int j) { return x[i] < x[j]; });
        vector<int> used(m);
        ll ans = 0;
        for (auto bag : bag_order) {
            for (auto box : boxes) {
                if (!used[box] && x[box] >= bags[bag].first) {
                    used[box] = 1;
                    ans += bags[bag].second;
                    break;
                }
