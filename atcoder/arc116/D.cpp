...
void solve(int) {
    ...
    int max_f = 3e5;
    vector<mint> fact(max_f);
    fact[0] = 1;
    for (int i = 1; i < max_f; ++i) fact[i] = fact[i - 1] * i;
    auto C = [&](int n, int m) { return fact[n] / (fact[n - m] * fact[m]); };
    ...
    for (int i = 1; i <= m; ++i) {
        // dp[i] = i <= n;
        for (int j = (i & 1); j <= i && j <= n; j += 2) {
            dp[i] += dp[(i - j) >> 1] * C(oldn, 2 * j);
        }
    }
...
void brute(int) {
    int n, m;
    cin >> n >> m;
    int k_max = 1 << (__lg(m) + 1);
    vector<vector<vector<mint>>> dp(n + 1, vector<vector<mint>>(m + 1, vector<mint>(k_max + 1)));
    dp[0][0][0] = 1;
    for (int i = 1; i <= n; ++i) {
