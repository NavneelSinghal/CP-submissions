...
int main() {
    ...
    for (int _test = 1; _test <= _tests; ++_test) {
        ...
        for (auto& x : b) cin >> x;
        int M = 5000;
        vector<mint> dp1(M + 1), dp2(M + 1);
        dp1[0] = 1, dp2[0] = 1;
        for (int mx = 1; mx <= 5000; ++mx) {
            // max a = mx
            // max b <= mx
            vector<int> l, le;
            for (int i = 0; i < n; ++i) {
                if (a[i] < mx && b[i] <= mx && !(a[i] < mx - 1 && b[i] <= mx - 1)) l.push_back(i);
                if (a[i] <= mx && b[i] <= mx && !(a[i] <= mx - 1 && b[i] <= mx - 1)) le.push_back(i);
            }
            // subset sums till b[i] = mx
            for (auto i : le)
                for (int j = M - b[i]; j >= 0; --j) dp1[j + b[i]] += dp1[j];
            for (auto i : l)
                for (int j = M - b[i]; j >= 0; --j) dp2[j + b[i]] += dp2[j];
            auto add = accumulate(begin(dp1), begin(dp1) + mx + 1, mint(0)) - accumulate(begin(dp2), begin(dp2) + mx + 1, mint(0));
            ans += add;
        }
        cout << ans << '\n';
    }
